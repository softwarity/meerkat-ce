...
| SSL-08 | **L'écran part des NOMS, pas des certificats.** La question d'un exploitant n'est jamais « quels certificats ai-je » mais « ma console répondra-t-elle, mon application répondra-t-elle ». Chaque plan déclare donc les noms qu'il doit servir - et les deux listes sont séparées parce que les deux plans ne se ressemblent pas : **une console a UN nom, une gateway applicative en sert PLUSIEURS**, c'est son métier. Pour chaque nom déclaré, la **couverture** est calculée en continu (couvert par tel certificat jusqu'à telle date, expire bientôt, expiré, en attente de l'autorité, ou rien ne répond) - en continu et pas au moment de l'ajout, parce qu'un contrôle qui n'a lieu qu'une fois ne dit rien le jour où quelqu'un supprime la mauvaise ligne. Le choix affiché est **celui que fait le moteur** à la poignée de main (un valide l'emporte sur un expiré, et parmi les valides le plus long), donc l'écran et la gateway ne peuvent pas se contredire. Chaque zone porte son propre menu d'ajout, **pré-rempli des noms non couverts** de ce plan : ils ont été déclarés une fois, et les retaper est exactement l'endroit où les deux divergent. | ✘ | S |
