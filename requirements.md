...
| ROUTE-03 | **Prédicats** : Path (avec/sans trailing slash), Host, Header, Cookie, Method, Query, RemoteAddr, XForwardedRemoteAddr, Weight, **`time-window`** (une seule brique là où il y en avait trois - after, before, between : personne n'en pose deux sur la même route, c'est la même fenêtre vue d'un bout, de l'autre, ou des deux ; hors fenêtre la route ne refuse pas, elle s'efface, donc il faut prévoir ce qui répond alors) ; au moins un prédicat obligatoire ; prédicat de **langue** (Accept-Language) ; exclusion par regex ; **`version`** : une PLAGE de versions d'API lue dans un en-tête ou un paramètre, `from` inclus et `to` exclu, comparée segment par segment comme des NOMBRES - c'est sa seule raison d'être, un `header` avec une regexp fait déjà l'égalité mais lit 1.10 comme plus petit que 1.9, et nommer toutes les versions donne une liste qui pourrit à chaque livraison. Une requête sans version ne matche pas : la route versionnée se pose AU-DESSUS de celle qui n'a pas de prédicat de version, et ce qui ne dit rien y atterrit. | ✔ | M |
| ROUTE-04 | **Filtres requête** : ajout/suppression/modification/**renommage** de headers (y compris « si absent »), **`set-host`** (écrit le champ ET l'en-tête - Go envoie l'un et tout le reste lit l'autre), set/add/remove de paramètres de query, **`remove-request-cookie`** (réécrit l'en-tête Cookie sans emporter la session), StripPrefix, PrefixPath (avec template `{language}`), RewritePath, **SetPath** (efface aussi la forme échappée, sinon c'est l'ANCIEN chemin qui part), **`copy-request-header`** (copie sans déplacer - un service lit le nouveau nom pendant qu'un autre a encore besoin de l'ancien), **`preserve-host`** (envoie le Host de l'appelant, pour un service qui construit ses liens et ses cookies avec le nom sous lequel on l'a demandé ; c'est l'en-tête Host posé par le filtre qui dit au routeur qu'on a DEMANDÉ un host, une comparaison de valeurs ferait de ce filtre-là un non-évènement), **`rewrite-query-param`** (regexp sur la valeur, un paramètre absent n'est pas inventé). **`X-Forwarded-Prefix`** : `strip-prefix` annonce au service le préfixe public qu'il vient de lui retirer (case « Tell the service », active par défaut). Sans lui, une application derrière un préfixe retiré voit `/orders` et écrit `/orders` dans TOUT ce qu'elle fabrique - liens d'une page, URLs d'un JSON, `path` d'un cookie, `servers` d'une spec - ce qui atterrit hors de la route ; `rewrite-location` ne rattrape que l'en-tête `Location`, donc les deux se complètent au lieu de se doubler. La valeur se calcule sur la requête RÉELLE (différence entre ce que l'appelant a demandé et ce qui reste), jamais sur le prédicat : une route peut porter plusieurs motifs, un motif peut commencer par un joker, et une route peut n'avoir aucun prédicat de chemin. L'en-tête entrant est **purgé sans condition**, même sur une route qui n'en pose pas : celui qui l'envoie choisirait sinon où le service écrit ses liens. Convention et non standard, comme `REMOTE_USER` (ROUTE-19). **Limites de taille** (corps et en-têtes), livrées comme des **gates** : une brique qui ACCEPTE ou REFUSE, phase à part du moteur (`phaseGate`), à côté des prédicats et de la règle d'accès sous la section Filters de la console. Un modificateur ne pouvait pas le faire : il transforme, sa signature ne porte ni erreur ni réponse, et il tourne dans le `Rewrite` du proxy où il n'y a pas de `ResponseWriter` pour répondre. Ce qui distingue une gate des deux autres décideurs : un prédicat qui ne matche pas et une règle d'accès qui n'accorde pas laissent la route SUIVANTE essayer, une gate qui refuse répond et s'arrête là. Deux briques plutôt qu'une (`max-request-body`, 413 ; `max-request-headers`, 431) : une brique, un refus, un statut, et pas de champ vide à interpréter - on retire la brique pour lever la limite. La taille s'écrit `2MB`, `512KB`, parce que c'est ce qui se relit dans un fichier exporté. Un corps annoncé trop gros est refusé sans être lu ; un corps non annoncé est coupé au passage et le refus reste un 413, jamais le 502 d'un amont qui n'a jamais été appelé. Le message nomme la limite et ce qui est arrivé : un « too large » nu envoie chercher dans l'application un réglage qui est dans la gateway. Un **Upgrade n'est jamais plafonné** : un WebSocket n'a pas un corps mais une conversation, et la couper à N octets serait le piège que la réécriture de réponse refuse déjà sur les flux. Les gates s'appliquent AUSSI à une route qui répond elle-même : elles ne transforment rien, contrairement aux modificateurs entrants qu'un terminal rend sans objet. | ✔ | M |
| ROUTE-05 | **Filtres réponse** : ajout/suppression/modification/**renommage** de headers, `set-status`, redirection, **`cache-control`** (pose Cache-Control et fait taire les Expires/Pragma qui le contrediraient), **`security-headers`** (nosniff, referrer, cadre, HSTS **sous TLS seulement**, CSP optionnelle), **`cookie-attributes`** (force Secure/HttpOnly/SameSite sur les cookies d'un amont qui ignore qu'il est derrière du TLS). **`remove-json-fields`** (suppression d'attributs par nom ou chemin pointé, **y compris dans les tableaux** - une liste d'utilisateurs garderait sinon tous les mots de passe qu'on lui a demandé de retirer). **`dedupe-response-header`** (la gateway et le service répondent parfois à la même question - CORS surtout ; `unique` garde l'ordre de première apparition, ces listes se lisent par position). **`rewrite-response-header`** (regexp sur la valeur, pour sortir un hôte interne de ce que le service raconte). **`rewrite-location`** : sans lui, un service derrière un préfixe retiré redirige le navigateur HORS de la route ; par défaut l'origine de l'amont est remplacée par celle que l'appelant a utilisée (reconstruite depuis les X-Forwarded-*, qui sont la seule chose qu'un filtre de réponse voit de la requête publique), et sans origine fiable on ne réécrit RIEN plutôt que d'envoyer le navigateur chez un hôte que personne n'a demandé. Reste à faire : le **cache de réponse local**, qui n'est pas un filtre mais un état entre les requêtes, comme les quotas (ROUTE-08). | ◐ | M |
...
| UIF-04 | **Canal WebSocket injecté** (socle livré) : les apps proxifiées reçoivent les notifications temps réel de la gateway sans intégration préalable. `GET /meerkat/events`, plan de données, et **rien qui nomme une fonctionnalité** : une enveloppe `{type, data}` que la page aiguille, parce qu'un canal nommé d'après son premier usage est la raison pour laquelle le second ouvre sa propre connexion. Ouvert **paresseusement**, au premier `meerkatPage.onEvent(type, fn)` : une page qui n'écoute rien ne paie rien, et c'est ce qui a permis de l'ajouter à des pages qui marchaient déjà. **Jamais requis** : l'état initial vient du `fetch` au chargement, le canal ne fait que le rafraîchir - un proxy d'entreprise qui mange les WebSockets coûte la fraîcheur, pas la fonctionnalité. La page se reconnecte seule, avec un recul exponentiel et de la gigue (une passerelle qui redémarre a toutes les pages de tous les navigateurs qui l'attendent, elles ne doivent pas revenir au pas). Une page qui prend du retard est **larguée** plutôt que mise en tampon : le remède à un message manqué est de se reconnecter et de refaire son fetch, et le navigateur le fait tout seul. Le contrôle d'origine est fait **ici** : un WebSocket échappe au CORS, et un sous-domaine voisin est same-site pour le cookie. Implémentation RFC 6455 maison (pas de dépendance) : le navigateur est le seul client, il n'envoie que des trames de contrôle. | ✔ | S |
...
| ID | Exigence | V1 | Prio |
|---|---|---|---|
| SSL-01 | **Un certificat entre par l'une de quatre portes**, et aucune n'est optionnelle : les lieux où tourne Meerkat ne se ressemblent pas. *Import* d'un couple PEM (ce qu'une autorité renvoie par mail) ou d'un keystore PKCS#12 (ce qu'exportent les mondes Java et Windows) ; *auto-signé* généré ici, du HTTPS en un clic pour un labo ou une première intégration, annoncé comme tel ; *demande de signature* générée ici, portée à l'autorité de l'entreprise et adoptée au retour - **la clé privée ne sort jamais**, et le certificat d'une autre commande est refusé à l'adoption, pas à la poignée de main ; *ACME* (SSL-05). Métadonnées X.509 extraites à l'entrée, chaîne, validité en compte à rebours, téléchargement, suppression. **Le remplacement est un échange de pointeur** : `tls.Config.GetCertificate` est appelé à chaque poignée de main, donc changer un certificat ne déplace aucun port, ne coupe aucune connexion, et **le SNI vient avec** (plusieurs certificats, choisi par le nom demandé, le plus vivant l'emportant pendant le chevauchement d'un renouvellement). Les clés privées sont **scellées au repos** sous la clé maîtresse du coffre. | ✔ | M |
| SSL-08 | **Un certificat appartient à un NOM.** La console a un nom et un certificat ; l'application a un nom par hôte qu'elle sert, et un certificat par nom. Le même matériel utilisé par les deux est **ajouté deux fois** - deux entrées et deux clés se comprennent mieux qu'un objet partagé plus la règle qui calcule quel nom il couvre. Il n'y a donc **pas de liste globale de certificats**, ni d'étoile de repli à désigner. **Et pas d'interrupteur HTTPS** : avoir un certificat, c'est ça l'activation ; le supprimer, c'est ça l'extinction - un interrupteur qui peut être « allumé » sans rien derrière est un interrupteur qui ment. Le formulaire d'ajout ne redemande **ni l'hôte ni un libellé** : l'hôte est déclaré une fois et il EST le nom de la chose - un second nom à saisir ne serait qu'un second nom à tenir en phase, jamais affiché nulle part, et un import qui **ne couvre pas** l'hôte sous lequel on le range est refusé **là**, pas à la première poignée de main. Les deux plans sont **semés avec `localhost`** - le nom par lequel toute installation répond le premier jour - via un **marqueur de semis à eux**, et non par l'absence du réglage : une installation qui avait déjà des réglages TLS écrits avant que les noms existent a la clé sans aucun nom, et resterait devant un bloc vide. Semé une fois, jamais rejoué à la lecture : un nom qu'on retire reste retiré, là où « vide veut dire localhost » le remettrait à chaque fois. **Chaque nom porte ses deux adresses**, en clair et en HTTPS, cliquables - sauf un joker, qui désigne des noms sans en être un. Le certificat de `localhost` reçoit `127.0.0.1` et `::1` en plus de son nom : on l'atteint par l'un ou par l'autre selon qui tape. | ✘ | S |
| SSL-02 | **Une porte HTTPS par plan, à côté du port en clair**, ouverte et fermée à chaud (`MEERKAT_TLS_ADDR`, `MEERKAT_ADMIN_TLS_ADDR`). Deux ports parce qu'ils ont deux métiers - l'un sert, l'autre dit où aller - et parce qu'un port dont on ne peut pas nommer le protocole est un port contre lequel personne n'écrit une règle de pare-feu, une sonde ou une procédure. Le plan admin a son propre interrupteur mais n'est pas secondaire : le même gestionnaire de sessions pose `Secure` d'après `r.TLS != nil`, donc un plan admin en clair est un cookie d'administration qui ne peut pas être marqué sûr. Ouvrir sans rien à présenter est **refusé en le disant**, et un port déjà pris est **nommé** sans que rien d'autre bouge. **HTTP/2** vient avec le TLS (ALPN), déclaré explicitement parce que cette écoute porte aussi le protocole du défi ACME. | ✔ | M |
| SSL-03 | **Un secret de TLS passe par le coffre** (VAULT-05). Le mot de passe d'un keystore n'est plus stocké du tout : il sert une fois à l'import, la clé est ré-encodée puis scellée - la V1 le gardait en clair à côté du keystore. Le secret qui reste est la **clé HMAC de compte externe** d'une autorité ACME d'entreprise, référençable par `$nom`. | ✔ | M |
| SSL-04 | Alerte (console + notification) avant **expiration des certificats**. La console affiche déjà le compte à rebours et signale une chaîne manquante ; ce qui manque est la **notification**, qui suppose que personne ne regarde l'écran. | ✘ | S |
| SSL-05 | **Émission et renouvellement automatiques par une autorité ACME**, et la porte est ouverte **sans dépendre de Let's Encrypt** : le répertoire est une **URL** (un step-ca interne, une autorité d'entreprise), avec le **certificat racine** qui signe ce serveur ACME et le **rattachement de compte externe** (EAB, RFC 8555) que la plupart des autorités privées exigent - de quoi tenir dans une salle sans accès internet. **Une seule autorité, un seul compte**, et une **case à cocher par nom déclaré** (SSL-08) : ce qui varie n'est jamais le plan mais le nom, et deux configurations ACME enregistreraient deux comptes chez la même autorité pour la même machine, pour rien. La liste fermée de domaines cesse donc d'être une troisième liste à tenir en phase - elle se déduit des cases. Un domaine posé par une autre voie (l'API, une configuration importée) est **adopté** dans les noms déclarés, jamais jeté en silence. Le défi est **TLS-ALPN-01**, joué sur le port 443 que la gateway tient déjà : le port 80 reste fermé. L'état ACME (clé de compte comprise) vit **scellé dans le stockage** et non sur disque, parce qu'il devra être partagé le jour du multi-instance. Changer d'autorité **oublie** le compte enregistré chez la précédente. Les conditions d'utilisation sont un **acte juridique** : rien n'est enregistré tant qu'une case n'a pas été cochée. | ✘ | C |
| SSL-06 | **Redirection du port en clair vers le port HTTPS**, sur le **plan de données uniquement**. Le statut suit la méthode : `301` pour GET et HEAD, que tout client comprend, `308` pour le reste, qui préserve méthode et corps - un `301` sur un POST le ferait rejouer en GET et perdrait le corps sans bruit. L'**hôte demandé est conservé** et seul le port est échangé : une gateway sert plusieurs domaines, répondre avec celui de sa configuration en enverrait la moitié ailleurs. **Deux choses ne sont jamais redirigées.** La sonde de vivacité : elle demande si le processus vit, pas s'il est protégé. Et **le port en clair de la console** : c'est depuis elle qu'on répare une configuration TLS cassée, donc elle ne doit jamais être ce que la casse emporte - avec deux ports ce n'est pas une règle à défendre, c'est une porte qu'on n'a simplement jamais câblée pour bouger. La redirection **se retire d'elle-même** si tout est périmé ou si la porte HTTPS n'a pas pu s'ouvrir. Reste **HSTS**, dont `security-headers` pose déjà l'en-tête sous TLS (ROUTE-05) : manque le réglage global qui l'arme - et il ne devra JAMAIS s'appliquer au plan de contrôle. HSTS retire au navigateur le droit de passer outre un avertissement de certificat, et il **ignore le port** (RFC 6797) : posé sur un hôte que les deux plans partagent, il rendrait la console inatteignable le jour d'une expiration. | ✘ | M |
| SSL-07 | **HTTP/3 (QUIC)**. Hors bibliothèque standard : une dépendance qui bouge vite, une seconde écoute en **UDP** (pare-feux d'entreprise, publication de port), un en-tête `Alt-Svc` sur les réponses TCP puisqu'un navigateur n'arrive jamais en h3 en premier, et les mêmes interdits que h2 en pire (pas de reprise de connexion, pas de WebSocket). Le gain est réel sur un front public consulté au mobile, marginal derrière un répartiteur d'entreprise. | ✘ | C |
...
| NOTIF-03 | **WebSocket serveur->client** : canal de notifications temps réel de la gateway vers les UIs (console et apps proxifiées via UIF-04), diffusion à un utilisateur ou broadcast, propagée entre nœuds du cluster. **Diffusion livrée** : l'audience est un **topic**, accordé une fois par connexion depuis la session - `all` (toute page ouverte, connectée ou non), `dev` (capacité développeur, DEV-01), `user:<id>` (le compte, à travers ses pages et ses navigateurs). L'abonné ne peut pas demander une salle où on ne l'a pas mis, donc **un producteur n'a jamais de question de permission à se poser** : il publie sur un topic. Un `POST /meerkat/events/echo` réservé à la capacité dev renvoie un message aux **seules pages de l'appelant** : c'est l'instrument du canal lui-même, sans quoi la seule façon de savoir si un socket traverse vraiment un proxy est d'attendre qu'une fonctionnalité ait quelque chose à dire. **Reste à faire** : la console (le canal est plan de données aujourd'hui) et la propagation entre nœuds - le hub est en mémoire, donc un message publié sur un nœud ne quitte pas ce nœud. | ✔ | S |
...
| ID | Exigence | V1 | Prio |
|---|---|---|---|
| DEV-01 | Le mode dev d'un utilisateur est **activé par un admin** (flag `dev`) ; à partir de là, le développeur accède à des fonctionnalités supplémentaires dans la console et sur la gateway. Au-dessus de cette capacité, un **commutateur d'installation** (Application, Général - réglage `dev_mode`, livré à ON) : éteint, il n'y a **plus aucune surface dev** sur les applications servies - ni menu Developer dans le user-button, ni mode test UI, ni doc API des routes, ni page profil dev, ni simulation d'identité côté data plane - quel que soit le nombre de comptes portant la capacité. La capacité dit **qui** peut développer, le commutateur dit si l'on développe **ici**. **Au-dessus des deux, l'ENVIRONNEMENT** (`MEERKAT_PRODUCTION`, livré) : cette passerelle est déclarée de production, et la surface dev y reste fermée quoi que dise la base. Le commutateur stocké **voyage** - un export de configuration, une sauvegarde restaurée, une base copiée depuis une préproduction peuvent chacun le rapporter à ON devant des clients, et personne ne le remarque avant qu'un port de tunnel soit ouvert. L'environnement, lui, ne voyage pas : c'est une propriété de l'endroit où le processus tourne. Il ne va que dans un sens (il ferme, il n'ouvre jamais ce qu'un exploitant a éteint), il le **dit** au démarrage, et la console **montre l'interrupteur désactivé avec la raison** plutôt que de cacher la carte : quelqu'un qui cherche son outillage doit apprendre pourquoi il n'est pas là. Une écriture qui tenterait de le rouvrir est **refusée** (422) et non acceptée puis ignorée. | ✔ | M |
...
| DEV-11 | **Cadre de sécurité** : l'admin définit quelles routes/services sont substituables et par quels devs ; substitutions et connexions plug sont listées dans la console et révocables à tout moment (y compris révocation d'une clé publique) ; toutes les actions (enregistrement de clé, pose/levée de substitution, opt-in/out) sont auditées. **Socle livré (2026-08-25)** : l'agent plug est embarqué dans l'image Enterprise (`ee/devplug`, `internal/devtunnel` pour l'état) - plug reste utilisable en agent autonome, c'est le mode par défaut de son propre dépôt et ce que fait l'image communautaire. **L'admission est une clé SSH publique déposée en self-service** (`/profile/dev/key`), pas un certificat signé : un certificat porte sa propre validité et resterait accepté jusqu'à expiration, alors qu'une passerelle est debout par définition et peut répondre « cette clé est-elle encore permise » à chaque connexion - **révoquer, c'est supprimer la ligne**, et ça prend effet en quelques secondes. Le nom du porteur revient avec le oui, d'où l'attribution (« checkout, par Alice ») que l'agent autonome ne peut pas donner : sa clé est partagée et prouve qu'on **a** plug, pas qui on est. **Le tunnel s'ouvre là où il peut travailler** : le mode développeur (DEV-01) dit que l'installation veut la surface dev, et le programme **regarde** s'il a ce qu'il faut plutôt que d'attendre qu'on le lui dise - un orchestrateur joignable, et un conteneur que cet orchestrateur connaît. Sur un poste de dev, ni l'un ni l'autre : rien ne s'ouvre et **une** ligne dit pourquoi. Le second test n'est pas redondant, car le premier ne suffit pas : Docker peut tourner sur le poste, plug identifie l'agent en inspectant son propre conteneur et une passerelle lancée depuis un arbre de travail n'en est pas un. `MEERKAT_PLUG_ADDR` vaut `:22222` par défaut et ne sert qu'à **déplacer** le port : on n'éteint pas une fonctionnalité en lui retirant son port, on ferme la surface dev (DEV-01). La passerelle **n'annonce rien** de son côté - l'agent dit « ready » sur son port quand il est réellement lié, et un port configuré n'est pas un port en écoute. L'identité SSH de l'hôte est **scellée dans le coffre** (VAULT-01) et non dans un fichier qu'un pod recréé jette. Un orchestrateur absent **dégrade** le tunnel, il n'arrête jamais la passerelle. **Reste à faire** : l'écran console (liste des connexions et des substitutions), l'audit des actions, et le cadre par route (quelles routes sont substituables, par qui). | ✘ | M |
...
| ID | Exigence | V1 | Prio |
|---|---|---|---|
| CFG-01 | La **configuration d'infrastructure** - routes, rôles, autorités, relais mail, thèmes, réglages de gateway - constitue une **Configuration** nommée. En sont exclus les **objets vivants** : utilisateurs, organisations, membres, secrets du Vault, certificats, sessions (gérés en continu, non affectés par un switch). Le partage passe donc par l'**intention et non la matière** : les noms qu'une gateway sert et l'autorité qui émet pour eux voyagent (SSL-08), les certificats jamais - ils portent une clé privée, et un document qui se dit public est le dernier endroit pour ça. Le contraste avec le **branding** est volontaire : un logo est une donnée en base64 dans un réglage, il voyage parce que c'est une décoration ; un certificat s'obtient là où il sert. Les configurations référencent les secrets par `$nom` (Vault), ce qui les rend portables. **Édition Enterprise** : ce qui se vend est la **taille de l'étagère**, pas le droit de s'en servir - toutes les opérations marchent sur les deux images, l'image communautaire en garde **trois à la fois** (compte affiché avant d'être atteint, refus qui dit combien tiennent). | ✔ | M |
