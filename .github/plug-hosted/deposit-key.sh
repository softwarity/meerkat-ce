...
id=$(curl -s --max-time 15 -b "$jar" "$admin/api/me" | python3 -c 'import sys,json;print(json.load(sys.stdin)["id"])')
curl -s --max-time 15f -b "$jar" -X PUT "$admin/api/users/$id/capabilities" \
