...
| [~] | AUTH-08 | **Cookie de session** | Session web par cookie httpOnly, Secure, SameSite ; domaine calculé pour être partageable entre sous-domaines de l'application ; identité | aucun attribut Domain, donc pas de partage entre sous-domaines | CE |
...
| [~] | AUTH-16 | **Code par e-mail** | Connexion par **code à usage unique envoyé par e-mail**, à la place du mot de passe (`/login/code`). Livré éteint, et l'interrupteur vit à côté de celui du repli MFA (écran Sécurité) : il fait de la **boîte mail l'identifiant**, donc l'allumer est une décision. Le cadre EST la fonctionnalité : **plan de données uniquement** - la console n'est jamais ouvrable par une boîte mail, les pages ne sont pas montées là ; code **lié au navigateur** qui l'a demandé (ce qui est stocké est le hash d'un identifiant de requête tiré au sort, posé en cookie HttpOnly, ET du code - donc un code lu au téléphone n'ouvre rien chez l'appelant) ; six chiffres, dix minutes, usage unique, un seul code vivant par compte ; **premier facteur seulement** - le second facteur tourne derrière comme derrière un mot de passe, puisque le code et un lien de réinitialisation voyagent par le même canal ; aucune énumération (même page, même statut, cookie posé même sans compte derrière) ; double limitation - envoi par adresse, essais par navigateur. L'historique de connexion nomme la porte (`code par e-mail`, et `code par e-mail + code` avec un second facteur). `code` devient un identifiant d'autorité réservé, refusé en nommant pourquoi : sinon l'autorité serait masquée par cette page sans que rien ne le dise | le lien magique (un clic au lieu d'un code), et interdire la porte par compte ou par rôle | CE |
...
| [x] | MFA-02 | **Code de secours par e-mail** | Un second facteur par e-mail, en **repli** : un compte déjà enrôlé en TOTP qui ne peut pas atteindre son application demande, au défi, un code par e-mail. Ne s'ouvre que si les quatre tiennent - compte enrôlé, adresse présente, option activée par l'admin (écran Sécurité, ships OFF), relais configuré - sinon le lien n'apparaît même pas. Code à 6 chiffres, 10 minutes, à usage unique, lié au compte (le hash porte l'identifiant ET le code) ; l'envoi est throttlé, la réussite efface le code en attente, et il se saisit dans le même champ que le code TOTP | - | CE |
...
| [x] | RBAC-06 | **Accès par route** | Contrôle d'accès par route, sur deux axes croisés (ET). Un refus sur une route UI **atterrit sur une page**, jamais sur une ligne de texte : le sélecteur d'organisation quand changer aiderait, la salle d'attente quand il n'y a aucune organisation, et sinon `/refused` - qui nomme la règle qui a écarté (organisation, rôle, compte nommé) et offre ce que cette session peut ouvrir. Une route de service garde son 403 : personne ne le lit dans un navigateur | - | CE |
...
| Fait | ID | Mot-clé | Description | Ce qui manque | Éd. |
|:---:|---|---|---|---|:---:|
| [x] | MODEL-01 | **Champs de compte** | Ce que l'installation sait d'une personne au-delà de ce que le produit a inventé : des champs définis une fois dans **Infra > Modèle** (texte, nombre, date, choix, oui/non - **jamais obligatoires** : un champ défini aujourd'hui est vide sur tous les comptes qui existent déjà, et l'exiger bloquerait le prochain qui en ouvre un pour autre chose), remplis par compte dans **Application > Utilisateurs**, et transmis comme n'importe quel autre fait sur l'appelant - en-tête ou JWT pour un service, attribut sur la page pour une UI. Définir et remplir sont deux gestes de deux métiers, et c'est ce qui rend le champ transmissible : personne ne peut s'attribuer une valeur qu'un amont croit. Une valeur hors de la liste d'un choix est refusée par une phrase qui nomme ce qui est permis, une route qui cite un champ inconnu aussi, et un champ retiré du modèle cesse de voyager au lieu de dormir dans la ligne | - | CE |
| [x] | MODEL-02 | **Fenêtre de validité** | Un compte porte ses dates d'accès (du / jusqu'au), en **jours et non en instants** : « jusqu'au 31 » vaut tout le 31. Hors fenêtre, la connexion est refusée en nommant la date, et la session en cours comme le plan de données le revérifient - un compte qui sort de sa fenêtre n'ouvre plus rien, sans que personne soit déconnecté en plein travail par une horloge. Ce qui approche est annoncé la veille plutôt que découvert au téléphone (NOTIF-04) | - | CE |
...
| [~] | ROUTE-08 | **Rate limiting** | Bornes par route et par consommateur, écrites dans la console. Le mot est **par**, pas **pour** : une règle dit sur quoi le compteur est clé (route, utilisateur, jeton, organisation, adresse) et les budgets se fabriquent seuls, un par appelant ; la seconde moitié dit **à qui** la règle s'applique, et c'est une `Access` - la même que les règles d'accès - donc un rôle devient un palier tarifaire sans nouveau concept. **Plusieurs sont vraies à la fois**, contrairement aux règles d'accès où la première qui matche gagne : 5000/min pour la route, 100/min par utilisateur, 60/min par adresse, la première dépassée répond. Fenêtre **glissante** (un seau qui se remet à zéro laisse passer le double à la bascule), en mémoire et par nœud - **dit sous les bornes**, là où le nombre se tape, parce qu'un chiffre qui veut dire autre chose en cluster que seul doit le dire à l'endroit où on l'écrit - avec un plafond de clés et un seau partagé au-delà - un compteur par adresse, c'est l'attaquant qui choisit combien. Les bornes sans identité refusent avant qu'une session soit cherchée | les quotas facturables (QUOTA-03) et le ralentissement (throttle) : le refus est un blocage | CE |
...
| [~] | ROUTE-20 | **gRPC** | gRPC de bout en bout : l'amont **déclare son transport dans son schéma** (`h2c://checkout:50051`), le seul endroit où le protocole d'un amont a sa place - un champ de plus aurait pu contredire le premier. `ForceAttemptHTTP2` ne suffisait pas : il monte en HTTP/2 par ALPN, donc sous TLS, et un amont en clair restait en HTTP/1.1 quoi qu'il annonce, ce qu'un service gRPC refuse. Le HTTP/2 en clair vient de la bibliothèque standard (`http.Protocols`, Go 1.24), donc **aucune dépendance ajoutée**. Les **trailers traversent**, ce qui est la moitié qui compte : un appel gRPC répond toujours 200 et met son verdict dans `grpc-status`, donc un proxy qui perd les trailers rend tous les échecs invisibles. Le reste ne change pas et les tests le disent un par un : un chemin `/pkg.Service/Method` traverse intact, les filtres d'en-tête s'appliquent (une métadonnée gRPC EST un en-tête HTTP/2), les règles d'accès valent comme ailleurs, et les transports se mutualisent par bornes - sur un protocole multiplexé ça compte plus, pas moins | la lecture de `grpc-status` par les métriques (aujourd'hui tout appel compte en 2xx, donc le taux d'échec d'une route gRPC lit zéro), la sécurité par méthode (RBAC-07 dérive d'un OpenAPI, gRPC a des descripteurs proto), et gRPC-Web | CE |
...
| [x] | UIF-03 | **Bouton utilisateur** | Bouton utilisateur flottant injecté dans les apps proxifiées : profil, déconnexion, changement d'organisation, et aussi bascule clair/sombre. Le mécanisme de cette bascule se déclare sur la route, parce qu'aucune application ne le lit pareil : **quel tag** la porte (html par défaut, mais une application qui lit son thème sur body ne le voyait jamais), puis un des trois modes. `set attribute` nomme UN attribut et lui donne deux valeurs (`data-theme="dark"`) ; `add attribute` et `class` prennent les deux noms eux-mêmes et posent le courant sur le tag - un attribut nu (`<body dark-theme>`) ou une classe. D'où le fait qu'une valeur vide veut dire quelque chose dans les deux derniers : rien sur le tag dans cet état, c'est-à-dire la forme la plus répandue - rien en clair, dark en sombre. Et quand une route **n'offre pas** la bascule, elle dit ce que le bouton porte lui-même (clair, sombre, ou le choix de la personne) : sans bascule il n'y a rien à suivre, et une application qui n'a qu'un seul aspect se retrouvait avec un bouton clair posé sur une page sombre. Ça habille le bouton seul, la page n'est jamais touchée | - | CE |
...
| [x] | UIF-06 | **CSS par route** | CSS additionnel par route (éditeur de code dans la console) et CSS conditionné par rôles : les rôles effectifs de la session sont posés sur le HTML servi, **côté serveur**. Une page ainsi estampillée appartient dès lors à UNE personne : la passerelle la rend non stockable (`no-store`, et les en-têtes qui pourraient dire l'inverse - `Expires`, `Pragma`, `Last-Modified`, `ETag` - sont retirés), quoi qu'en dise l'application derrière. Sans quoi un `public, max-age=300` - le réglage par défaut de tout serveur de fichiers statiques - ferait servir la page d'Alice à Bob par un CDN, un proxy d'entreprise ou un navigateur partagé. Une requête anonyme n'est pas estampillée et garde la mise en cache de l'application | - | CE |
...
| [x] | PORTAL-01 | **Portail de navigation** | Une barre de navigation injectée dans les pages proxifiées (en-tête OU rail, l'axe primaire se bascule - ce que `portal-nav` d'understory ne savait pas faire), qui promeut le sous-menu « Applications » du bouton utilisateur en vraie navigation : des modules parents et leurs enfants, un lanceur en gaufre, des flèches de débordement, le logo de la marque et le bouton utilisateur intégrés dans la barre. Réglage GLOBAL (comme le thème, THEME-01), édité sous Application avec un aperçu-maquette live. Quand il est actif, il remplace le bouton utilisateur par route (qui migre dans la barre et y perd son sous-menu Applications) sur toutes les routes UI ; chaque module se lie à une route UI et hérite de son accès, donc un visiteur ne voit que ce que ses droits autorisent (le payload ne porte aucune règle d'accès). Composant vanilla `meerkat-portal-nav` (const Go, shadow DOM) habillé du thème du plan de données et du color-scheme du visiteur, servi comme le bouton utilisateur ; jamais affiché dans une iframe (self !== top) | l'atterrissage sur la première appli accessible ; le canal de badge (le champ existe, pas la mécanique de push, prévue sur le bus WS) ; le drag-and-drop du catalogue (réordonner, glisser pour changer de niveau - P3) ; la personnalisation par tenant (PORTAL-02) | CE |
...
| Fait | ID | Mot-clé | Description | Ce qui manque | Éd. |
|:---:|---|---|---|---|:---:|
| [x] | NOTIF-01 | **SMTP** | SMTP configurable à chaud (host, port, smtp/smtps, STARTTLS, auth, mot de passe via Vault), avec test d'envoi depuis la console. Un **gabarit unique** habille tous les envois aux couleurs du thème (palette claire, styles inline sur table, texte + HTML, logo de l'intégrateur mais jamais la marque Meerkat) : confirmation de compte, mot de passe oublié, avis de changement de mot de passe, code de second facteur (MFA-02), résumé des expirations (NOTIF-04). Anglais rempli, français traduit, les autres langues retombent sur l'anglais | - | CE |
...
| [x] | NOTIF-04 | **Résumé des expirations** | Le seul message que la passerelle envoie d'elle-même : une fois par jour, les administrateurs qui portent une adresse (root et app admin) reçoivent les comptes dont la fenêtre d'accès se ferme dans les N jours et ceux qui viennent de se fermer, avec la date - **la dernière journée valide**, celle qu'un administrateur retaperait. Un jour sans rien à dire n'envoie rien : un message quotidien qui répète « rien aujourd'hui » trois cents fois n'est plus ouvert le jour où il compte. Réglé là où vit le relais (interrupteur, heure, horizon) parce que c'est la même question - ce qui sort d'ici par e-mail - et parce que sans relais c'est un interrupteur derrière lequel il n'y a rien : le jour est alors laissé **non coché**, donc le premier matin où un relais répond, l'avis part. Un seul envoi par jour quoi qu'il arrive au processus : le jour envoyé est dans la base et un verrou consultatif tranche entre les nœuds, donc un redémarrage à 07h59 n'en émet pas un second et un cluster de quatre en émet un, au **même gabarit thémé** que les autres mails (NOTIF-01) | - | CE |
...
| [~] | CONSOLE-07 | **Utilisateurs** | Gestion des utilisateurs par le root : recherche, capacités basculées depuis la ligne, réinitialisation de mot de passe (temporaire, affiché une seule fois). **Un seul tiroir, dont le contenu change** : créer un compte et le corriger sont la même surface, les mêmes champs dans le même ordre - un compte qui naît dans une petite boîte de dialogue et se complète ailleurs enseigne deux écrans pour un objet, et laisse ses champs obligatoires vides entre les deux. Le tiroir tient **trois pages** : le compte (identité, fenêtre d'accès, champs maison - ce qu'on REMPLIT), la **sécurité** (second facteur, mot de passe, autorités externes) et l'**historique de connexion**, chacune atteinte par une porte pleine largeur et rendue par une flèche de retour. Un clic dehors - ou Échap - rend exactement un niveau : la sous-page rend le compte, le compte rend la table | la pagination serveur, l'import en masse, un délai de validité sur le mot de passe temporaire | CE |
...
| [x] | CONSOLE-12 | **Écran des routes** | La liste filtre sur ce qu'on sait d'une route (nom, amont, chemins) et sur son **genre** - toutes, UI, service - parce qu'une route UI est un autre métier que les services autour d'elle et qu'à 157 routes le marqueur à côté du nom n'aide qu'une fois la ligne trouvée. Une route UI porte en plus un bouton qui l'**ouvre dans le plan de données**, à l'adresse que la passerelle donne elle-même (jamais celle de la console : un port publié ou un ingress s'intercale). Le lien est suivi par le navigateur et non appelé en fetch, pour que l'appelant atterrisse sur la vraie réponse de la passerelle - la page de connexion, ou celle qui dit pourquoi (RBAC-06) | - | CE |
...
| Fait | ID | Mot-clé | Description | Ce qui manque | Éd. |
|:---:|---|---|---|---|:---:|
| [~] | QUOTA-01 | **Quotas** | Quotas API définissables par route et par consommateur (utilisateur, token d'application, organisation, IP) : limites en nombre de requêtes | les compteurs qui se facturent ou se montrent (QUOTA-03) : ceux-là sont en base, écrits par lots, et c'est un autre mécanisme | CE |
| [~] | QUOTA-02 | **Réponse 429** | Dépassement -> 429 avec les headers standards (RateLimit-*, Retry-After) ; politique configurable par quota : bloquer, ralentir (throttle) | le ralentissement (throttle) : le dépassement bloque, il ne ralentit pas | CE |
...
| [x] | QUOTA-05 | **Quotas par endpoint** | Quotas par endpoint (méthode + path) : posés sur l'inventaire d'endpoints de la route, celui que la gateway lit dans son OpenAPI (SVC-06) | - | CE |
...
| [x] | SEC-10 | **Rien de personnel en cache** | Toute réponse dans laquelle la passerelle écrit une identité est rendue non stockable : les pages estampillées (UIF-06), le bandeau de maintenance d'un administrateur qui a pris la porte (LIFE-05), les pages du flux (`writeFlow`), la charge utile du bouton utilisateur, les réponses `respond` qui rendent un gabarit d'identité. Reste à la charge de l'application ce qu'elle personnalise elle-même à partir des en-têtes reçus : le filtre `set-cache-control` est là pour le dire à sa place | - | CE |
...
| [x] | PERF-05 | **Bancs de mesure** | `internal/gateway/bench_test.go` : le proxy nu de la bibliothèque standard comme plancher, puis la route minimale, la table de 50, les filtres, le terminal, et la **sélection isolée** du socket sur 1 / 50 / 200 routes. Le chiffre absolu dépend de la machine et ne s'écrit nulle part ; ce qui se lit est le **surcoût relatif** au plancher mesuré dans la même exécution, et le **nombre d'allocations**, qui est un compte et non une durée. Un test bloque la régression en énonçant une **forme** et non un plafond : matcher contre 200 routes doit allouer exactement ce que matcher contre 1 alloue. Un banc de plus couvre la **requête authentifiée** (jeton, identité, JWT signé), le seul chemin qui lit la base. **Banc comparatif** (`tools/bench`, `make bench`) : Meerkat face à Kong, APISIX et Traefik libres, plus une **référence** qui n'est pas un produit - le proxy nu de la bibliothèque standard Go, qui dit ce que le langage permet -, chacun épinglé sur **un CPU** devant le même amont, sous la même charge (`oha`, débit fixe avec correction de latence, puis débit maximal), dans la même exécution ; chaque scénario est vérifié avant d'être mesuré (200 avec le jeton, refus sans), et une mesure sous 99 % de 200 est refusée plutôt qu'enregistrée. Trois scénarios **comparables** (proxy, jeton vérifié par la gateway, limitation de débit) et un **tel que déployé** (l'authentification déléguée de Traefik, ForwardAuth, et son rebond réseau mesuré contre le service le plus rapide possible). La CI le joue à chaque commit sur main, sur le miroir, sur les runners **x64 et arm64** (4 CPU, 16 Go), relève la machine (`lscpu`) et réécrit `latest.json` sur la branche `bench` (la dernière mesure seulement) ; la page **Performance** du site de doc la lit à chaque visite et explique le protocole, la configuration des autres produits, l'écart à la référence Go et les conditions des runners. Il a trouvé dès ses premières exécutions cinq plafonds du produit, corrigés : le pool vers l'amont ne gardait que 8 connexions (et des 502 sous charge longue), chaque réponse proxifiée allouait un tampon de 32 Ko, chaque requête était clonée même sans rien à retirer - proxy nu de 4 164 à environ 40 000 req/s sur un cœur - ; chaque requête par jeton relisait la base, et chaque identité transmise relisait compte, organisation, appartenances et rôles - requête authentifiée de 4 200 à 11 000 req/s | - | CE |
...
| [x] | PERF-03 | **Cluster actif/actif** | Cluster actif/actif (optionnel, motivé par la haute disponibilité - la gateway est la porte d'entrée unique) : N nœuds sans état local. Bus de changement (STORE-03), **verrou consultatif** `pg_advisory_lock` autour de l'émission ACME et de l'entretien périodique, caches de sessions et canal live relayés (PERF-04). **Derrière le répartiteur** : aucune affinité de session à demander (les sessions vivent en base, le cache de 5 s est invalidé par le bus) et aucun nœud primaire à viser (le verrou consultatif garde ce qui ne doit se faire qu'une fois) ; en échange, le répartiteur sonde `/readyz` et jamais `/healthz` (OBS-02), et le schéma se lit sur `X-Forwarded-Proto` autant que sur la connexion - une terminaison TLS en amont laissait sinon les cookies sans `Secure`, la console annonçait des URL `http://` et la redirection vers HTTPS bouclait à l'infini | - | EE |
| [x] | PERF-04 | **État partagé** | Tout état de flux est partagé ou invalidé entre nœuds : clés TOTP et passkey, jetons OAuth, **clé de simulation** (un jeton frappé par un nœud vaut sur tous) et compteur anti-force brute en base ; le **cache de sessions** (5 s) et celui des **jetons d'API** (même fenêtre, vidé par toute écriture sur le jeton, son propriétaire ou la politique), l'**identité transmise** (compte, organisation, appartenances, rôles : une époque avancée après toute écriture servie hors trafic proxifié et relayée par le bus), le **hub d'événements** et les **noms servis depuis un poste de développeur** restent en mémoire mais sont invalidés ou relayés par le bus - pour ce dernier, le trafic traversait déjà seul (le panneau de plug est un service de l'overlay qui relaie vers la tâche tenant la session), donc ce qui manquait était de le **dire** : sur un autre nœud, le code d'un collègue arrivait sans qu'un mot le signale | - | CE |
...
| [x] | OBS-05 | **Prometheus** | **Enterprise**, et le découpage est celui-ci : les compteurs et les tableaux de bord intégrés sont dans les deux éditions - c'est la promesse « zéro dépendance », et la CE a des courbes sans rien installer ; ce qui se vend est **l'externalisation** vers la stack de monitoring que le client a déjà. `/metrics` sur le plan de contrôle (`ee/prometheus`, absent de l'image communautaire : du code absent refuse tout seul), derrière la garde `infra-admin` (les compteurs nomment chaque route et chaque gabarit : c'est une carte de l'installation, pas une page publique), un **interrupteur livré éteint**, et un jeton de portée `metrics` qui n'ouvre que ce chemin - une crédentiale de scraper vit dans la configuration d'une stack de supervision, souvent le dépôt d'une autre équipe, l'endroit où un jeton a le plus de chances de fuiter et le moins d'être tourné. Format texte : totaux monotones, histogramme cumulatif, un label `source` qui sépare les gabarits déclarés des déduits. Le tiroir de l'écran Metrics porte l'interrupteur et les fichiers à écrire - Swarm, Kubernetes, Grafana - qui sont de **vraies ressources** servies par la passerelle (`/monitoring/swarm/prometheus.yml`...), donc éditables comme des YAML et récupérables sans passer par la console ; copiables et téléchargeables, avec le port d'écoute de **cette** installation dedans - pas celui par lequel la console a été jointe, puisqu'une publication de port ou un ingress s'intercale et que le scrape, lui, adresse le conteneur - et le vrai nom du réseau, que la découverte (SVC-02) lit sur le socket. Un **seul** `prometheus.yml` : le scrape ne change pas d'une plateforme à l'autre, seule la découverte de la cible change, donc le fichier porte les deux, commentées, et un bouton par plateforme rend la version voulue - son bloc décommenté, l'autre supprimé. Un test parse chaque version produite et refuse celle qui ne saurait pas où chercher. Les deux versions visent **chaque nœud** et non le service devant eux (`tasks.meerkat` en Swarm, `role: pod` en Kubernetes) : les compteurs sont par nœud, donc scraper une VIP tirerait un nœud différent à chaque passage et dessinerait une courbe qui n'est celle de personne. Côté Grafana, la source de données ET **un tableau de bord provisionné** (dix panneaux : trafic par route, taux d'échec, p95, endpoints les plus lents et les plus coûteux, services muets, connexions refusées) - le compose les monte, Grafana démarre avec Meerkat déjà dessiné. Et **sans seconde connexion** : le compose éteint le formulaire de Grafana et son Basic et la passe en mode proxy d'authentification, donc la route décide qui entre et l'identité transmise en en-têtes (SAUTH-01) dit qui c'est. Ce qui fait de la non-publication du port une règle et non plus un confort : l'en-tête est désormais toute l'authentification, et la passerelle purge celui qu'un appelant enverrait pour se prétendre quelqu'un. Prometheus **tire** (scrape), il ne reçoit pas : la vue intégrée est donc plus temps réel que lui, pas une version dégradée | - | EE |
...
| [~] | DEPLOY-07 | **Fichiers de déploiement** | Les fichiers qui déploient chaque forme sont livrés et **téléchargeables depuis le site** (`/deploy/`), copiés du dépôt à chaque publication : le chart Helm couvre les trois formes - une passerelle communautaire sur son volume, une passerelle Enterprise avec le tunnel, trois passerelles Enterprise sur un PostgreSQL partagé (répliques, RollingUpdate sans passer sous le compte, anti-affinité, plus de volume) - et il **refuse** plusieurs répliques sans base partagée en nommant quoi poser. Côté Docker, le compose communautaire, le compose Enterprise avec le tunnel, et une pile Swarm à trois tâches dont les trois secrets sont interpolés au déploiement plutôt qu'écrits dans le fichier | un dépôt Helm indexé (`helm repo add`), donc une installation par URL plutôt que par archive | CE/EE |
...
| [~] | STORE-07 | **Montée de version** | Deux moitiés. L'**additive** tourne à chaque démarrage et n'a besoin d'aucun registre : le DDL de la version est appliqué en `CREATE TABLE IF NOT EXISTS`, puis `addMissingColumns` ajoute les colonnes que la base n'a pas - une table, une colonne, un index arrivent sans que personne écrive une étape. L'autre moitié est ce que cette moitié ne sait pas dire : une colonne renommée, un type changé, une valeur remodelée, une ligne rétro-remplie. Ces étapes-là sont **ordonnées, jouées une fois, enregistrées au fur et à mesure** (`internal/store/migrations.go`), donc une montée interrompue reprend où elle s'est arrêtée. Une base écrite par une version **plus récente** est refusée en nommant les deux numéros : revenir en arrière est normal quand une release tourne mal, et c'est exactement là que le refus vaut mieux que la perte. Le registre s'ouvre vide, et c'est légitime tant qu'aucune version n'a été publiée | l'épreuve qui compte : monter une base **amorcée par une version antérieure** en CI, ce qui suppose d'abord une première release à monter depuis | CE |
| [x] | STORE-03 | **Cluster** | Mode cluster = base externe partagée, et c'est **le pilote qui est l'édition** : `ee/pgdriver` porte l'ouverture de la connexion et l'attente des notifications, donc l'image communautaire n'a aucun moyen de parler à PostgreSQL - `MEERKAT_DATABASE_URL` y reçoit une phrase qui nomme ce qui est disponible, pas une erreur de pilote. Du code absent refuse tout seul, aucune licence n'est lue. Ce qui reste dans la couche de stockage est la portabilité du schéma (la réécriture des `?`, quatre requêtes d'introspection) : inutile sans connexion, et pas de quoi couper un paquet en deux.  la synchronisation inter-nœuds passe par un **bus de changement** (`internal/cluster`) : une version par sujet en base plus un `NOTIFY` pour ce qui se recharge (routes, certificats), un signal porteur pour ce qui s'invalide (sessions) ou se relaie (canal live), et un **verrou consultatif** (`Store.WithLock` / `TryLock`) pour ce qui ne doit se faire qu'une fois | - | EE |
...
| Fait | ID | Mot-clé | Description | Ce qui manque | Éd. |
|:---:|---|---|---|---|:---:|
| [x] | QUAL-01 | **Tests** | Tests automatisés significatifs sur les chemins critiques (auth multi-étapes, RBAC, routage, filtres de corps), la **même suite tournant deux fois** : sur base embarquée, et sur un vrai PostgreSQL en CI (`internal/store/dbtest` donne à chaque test son schéma, et rend la main quand aucun serveur n'est nommé - donc `go test ./...` sur un poste reste intact). La passe PostgreSQL vit sur l'arbre qui sait s'y connecter, avec la suite du bus de changement qui n'a de sens que là - **dont deux vraies passerelles**, deux processus contre une base : une route écrite par l'API de l'une est servie par l'autre, et sa suppression traverse aussi. C'est la seule chose qu'aucun test mono-processus ne peut répondre, et elle coûte deux secondes dans un job qui avait déjà monté le serveur. Le **tunnel de développement** a le sien, hebdomadaire et déclenchable : un vrai Swarm d'un nœud, le client installé par le compte anonyme de l'image, la clé du profil déposée par un compte développeur, et une reprise de nom vérifiée depuis l'intérieur du cluster - l'agent s'identifie en inspectant son conteneur, donc rien de tout ça n'est atteignable sans orchestrateur. Une cadence et non un garde-barrière : ce qu'il exerce bouge quand plug sort une version, pas quand un commit atterrit ici | - | CE |
...
### Les quotas : par, pas pour (ROUTE-08, QUOTA-01 à 05)

Le mot qui a débloqué la conception est **par**, pas **pour**. Personne n'écrit une limite *pour*
alice : une liste de noms ne se maintient pas et ne répond pas à la question. Une règle dit sur
quoi le compteur est **clé** - la route, l'utilisateur, le jeton, l'organisation, l'adresse - et
les budgets se fabriquent tout seuls, un par appelant. La seconde moitié dit **à qui** la règle
s'applique, et c'est une `Access`, la même que les règles d'accès : « 1000/min par utilisateur
pour qui porte le rôle partner » ne nomme personne, et le rôle devient un palier tarifaire sans
qu'on invente un concept.

**Plusieurs s'appliquent en même temps**, et c'est là que ça diffère d'une règle d'accès où la
première qui matche gagne. Ce sont des bornes, on les veut toutes : 5000/min pour la route
protège le service, 100/min par utilisateur empêche un appelant de tout prendre, 60/min par
adresse couvre ceux qui n'ont pas de compte.

**Corollaire, et il se voit à l'écran** : une règle « only for » borne les appelants qu'elle
décrit, et **personne d'autre**. Trois règles étroites font une route qui a l'air bornée et qui
est grande ouverte à tous ceux que les trois ne décrivent pas - exactement le cas qu'on croit
avoir couvert. L'écran le dit quand aucune borne n'est pour tout le monde, et un test fixe la
sémantique.

**Point dur, résolu comme prévu** : un compteur exact et partagé coûte un aller-retour par
requête, inacceptable sur le chemin de données. Donc une **fenêtre glissante locale** au nœud
pour la protection - un seau qui se remet à zéro laisse passer le double à la bascule, ce qui
n'est pas une approximation mais une faute - et il reste à faire le **compteur en base** pour les
quotas qui se facturent (QUOTA-03), écrits par lots.

**Et la cardinalité, encore.** Une borne par adresse fait un compteur par adresse, et c'est celui
qui envoie les requêtes qui choisit combien : un botnet ferait grossir la mémoire de la gateway
*par le mécanisme installé pour survivre à un botnet*. Donc les clés sont plafonnées, les
expirées balayées d'abord, et la longue traîne partage **un** compteur - refuser les non suivis
laisse n'importe qui couper le service en tournant d'adresse, les laisser passer laisse
n'importe qui contourner la borne de la même façon.

**Où** la borne est vérifiée est aussi une décision : celles qui n'ont besoin d'aucune identité
refusent avant qu'une session soit cherchée, comme les portes de taille ; celles qui sont clées
sur un utilisateur coûtent une résolution de session, payée seulement par les routes qui en
demandent une.

**Ce qui reste** : le ralentissement (throttle) - le dépassement bloque, il ne ralentit pas - et
les compteurs facturables de QUOTA-03.

### Il n'y a aucune restauration (STORE-05)

(La montée de version, elle, est STORE-07 : deux lignes portaient STORE-05, et un identifiant
réutilisé est exactement ce que le contrat interdit - c'est ce qui fait qu'une case cochée
ailleurs cesse de vouloir dire quelque chose.)
